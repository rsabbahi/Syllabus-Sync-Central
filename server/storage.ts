...
export interface IStorage {
  ...
  importEventsAsTasks(userId: string, connectionId: number | null, events: NormalizedEvent[]): Promise<{ imported: number; skipped: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteImportedEventsByConnection(connectionId: number): Promise<void> {
    const rows = await db.select().from(calendarImportedEvents)
      .where(eq(calendarImportedEvents.connectionId, connectionId));

    const taskIds = rows.map(r => r.taskId).filter((id): id is number => id !== null);
    if (taskIds.length > 0) {
